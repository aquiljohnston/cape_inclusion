...
export function Employees() {
  return (<>
    <div className="step-header">
      ...
      <h5>2. Enter the number of employees for each category</h5>
    ...
    <table>
      ...
      <tbody>
        {MODEL.map((item, index) => (
          <tr key = {`Employees${index}`} style={{backgroundColor: index%2 ? 'white': 'whitesmoke'}}>
            <td className="td-title">{item.title}</td>
            {[...Array(5).keys()].map(item_index => (
              <td key={`Employees.${item.name}_${item_index}`}>
                <Field                
                  name={`Employees.${item.name}.${item_index}`}
